...
- 동일 역할·Provider의 소유자 Agent 재사용 계획과 `REUSE_AGENT`, `ADD_AGENT`, `REWIRE_STEPS` 변경 계약
- 요약·번역·분류처럼 단일 책임인 업무는 한 명만 제안하고, 독립 책임이 증명될 때만 구성원을 추가

완료 기준: API 키 없이 Stub으로 전체 설계·승인을 통합 테스트할 수 있고, 실제 경로는 사용자의 ACTIVE 자격증명이 없으면 시작되지 않는다.

### 3C. 검증된 불변 하네스 실행

- 설계기와 분리된 deterministic `HarnessValidator`
- 시작·종료·순차 Edge·Agent 소유권·재시도·타임아웃·승인·결과 계약 검사
- 발행 버전에 Validator 버전, 구조 해시와 검사 결과 기록
- Execution 생성 시 발행 버전 전체를 자격증명 없이 불변 스냅샷으로 고정
- 자격증명 참조는 API 비노출 내부 binding으로 분리
- 승인과 Worker 종료가 겹치는 Queue 재개 race 방지

완료 기준: 발행 뒤 Draft를 수정해도 기존 버전 실행이 변하지 않고, 새 구조는 재검증·재발행 후에만 실행되며 승인 후 실행이 자동 재개된다.
