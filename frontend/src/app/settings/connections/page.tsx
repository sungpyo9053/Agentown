...
export default function ConnectionsPage() {
  ...
  return <AppShell kicker="SETTING · CONNECTORS" title="업무 연결">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <article className="border border-hairline bg-white p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold tracking-[.14em] text-coral">READ CONNECTOR</p><h2 className="mt-2 text-2xl font-medium">Notion</h2><p className="mt-2 text-sm text-mute">공유된 페이지·데이터 소스 검색</p></div><span className={`rounded-pill px-3 py-1 text-xs ${notion.data?.connected ? "bg-green-100 text-green-800" : "bg-cloud text-charcoal"}`}>{notion.data?.connected ? "연결됨" : notion.data?.configured ? "연결 가능" : "서버 설정 필요"}</span></div>
