...
export default function ConnectionsPage() {
  ...
  const notionNeedsReconnect = notion.data?.connections.some(item => item.status === "INVALID") ?? false;
  ...
  return <AppShell kicker="SETTING · CONNECTORS" title="업무 연결">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <article className="border border-hairline bg-white p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold tracking-[.14em] text-coral">READ · APPROVED WRITE</p><h2 className="mt-2 text-2xl font-medium">Notion</h2><p className="mt-2 text-sm text-mute">공유된 페이지 검색 · 미리보기 승인 후 새 페이지 발행</p></div><span className={`rounded-pill px-3 py-1 text-xs ${notion.data?.connected ? "bg-green-100 text-green-800" : notionNeedsReconnect ? "bg-amber-100 text-amber-900" : "bg-cloud text-charcoal"}`}>{notion.data?.connected ? "연결됨" : notionNeedsReconnect ? "다시 연결 필요" : notion.data?.configured ? "연결 가능" : "서버 설정 필요"}</span></div>
        {notion.data?.connections.map(item => <div key={item.id} className={`mt-4 border p-4 ${item.status === "INVALID" ? "border-amber-300 bg-amber-50" : "border-hairline"}`}><div className="flex items-center"><div><b>{item.workspaceName}</b><p className="mt-1 text-xs text-mute">{item.status === "INVALID" ? "다시 연결 필요" : item.status}{item.lastVerifiedAt ? ` · 읽기 검증 ${new Date(item.lastVerifiedAt).toLocaleString("ko-KR")}` : item.status === "INVALID" ? "" : " · 읽기 검증 전"}</p></div><button onClick={() => revokeNotion.mutate(item.id)} className="ml-auto text-sm font-medium text-sale">연결 해제</button></div>{item.status === "ACTIVE" && <button onClick={() => verifyNotion.mutate(item.id)} disabled={verifyNotion.isPending} className="mt-3 rounded-pill border border-ink px-4 py-2 text-xs font-medium disabled:opacity-40">실제 읽기 검증</button>}{item.status === "INVALID" && <div className="mt-3"><p className="text-xs leading-5 text-amber-900">Notion 연결이 만료되었습니다. 다시 연결한 뒤 보존된 실행을 재시도해 주세요.</p><button onClick={() => connectNotion.mutate()} disabled={!notion.data?.configured || connectNotion.isPending} className="mt-3 rounded-pill bg-ink px-4 py-2 text-xs font-medium text-white disabled:opacity-40">Notion 다시 연결</button></div>}</div>)}
