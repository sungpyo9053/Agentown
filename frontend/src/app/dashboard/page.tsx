...
type Home = { id: string; handle: string; title: string; introduction?: string; visitCount: number; items: unknown[] };
...
const faces: Record<string, string> = { writer: "✍️", reviewer: "🔎", designer: "🎨", developer: "💻", manager: "🧭" };
...
export default function DashboardPage() {
  ...
  return <main className="mx-auto max-w-6xl px-6 pb-20 pt-8">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex gap-2"><button onClick={() => setCredentialOpen(!credentialOpen)} className="rounded-full border bg-white px-5 py-3 font-bold">🔐 API 키</button><button onClick={() => setAgentOpen(!agentOpen)} className="rounded-full bg-coral px-5 py-3 font-bold text-white">+ 새 구성원</button></div>
    </div>
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_310px]">
      <div className="overflow-hidden rounded-[2rem] border-[8px] border-white bg-white shadow-card">
        <div className="room-grid relative min-h-[520px] p-7">
          <div className="rounded-2xl bg-white/85 px-5 py-4 backdrop-blur-sm"><b>{home.data?.title}</b><p className="text-sm text-stone-500">{home.data?.introduction || "우리 마을의 첫 이야기를 적어보세요."}</p></div>
          <div className="absolute inset-x-0 bottom-0 h-[42%] bg-[#bd9268] [clip-path:polygon(0_23%,100%_0,100%_100%,0_100%)]" />
          <div className="relative z-10 mt-28 grid grid-cols-2 gap-6 sm:grid-cols-3">
            {(agents.data ?? []).map((agent) => <article key={agent.id} className="text-center">
              <div className="mx-auto grid h-24 w-24 place-items-center rounded-[2rem] bg-white text-5xl shadow-lg">{faces[agent.characterKey] ?? "🤖"}</div>
              <div className="mx-auto mt-2 w-fit rounded-xl bg-white/95 px-4 py-2 text-sm shadow"><b>{agent.name}</b><br /><span className="text-xs text-stone-500">{agent.role}</span></div>
            </article>)}
            {!agents.isLoading && agents.data?.length === 0 && <button onClick={() => setAgentOpen(true)} className="col-span-full rounded-3xl border-2 border-dashed border-leaf/30 bg-white/60 p-10 font-bold text-leaf">첫 AI 구성원을 초대하세요 +</button>}
          </div>
        </div>
      </div>
...
function AgentForm({ credentials, done }: { credentials: Credential[]; done: () => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    mutation.mutate({ name: data.get("name"), role: data.get("role"), characterKey: data.get("characterKey"), script: data.get("script"), guide: data.get("guide"), modelProvider: data.get("provider"), modelName: data.get("model"), credentialId: data.get("credentialId") || null, maxOutputTokens: 2048, timeoutSeconds: 60, visibility: "PRIVATE" });
  };
  return <form onSubmit={submit} className="mt-7 grid gap-4 rounded-3xl bg-white p-6 shadow-card md:grid-cols-2">
    ...
    <Field label="캐릭터"><select name="characterKey">{Object.entries(faces).map(([key, face]) => <option key={key} value={key}>{face} {key}</option>)}</select></Field>
