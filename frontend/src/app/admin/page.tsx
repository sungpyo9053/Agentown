...
export default function AdminPage(){
 ...
 return <AppShell kicker="PLATFORM ADMIN" title="Agentown 전체 운영 콘솔">
  <div className="rounded-[2rem] bg-ink p-6 text-white"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black tracking-[.2em] text-coral">SYSTEM CONTROL</p><h2 className="mt-2 text-2xl font-black">서비스 전체 상태</h2><p className="mt-2 text-sm text-stone-300">개인 결과물 내용은 열람하지 않고 운영 메타데이터만 관리합니다.</p></div><div className="rounded-2xl bg-white/10 px-5 py-3 text-right"><b className="text-leaf">● {system.data?.status??"확인 중"}</b><small className="mt-1 block text-stone-300">{system.data?.workerMode??"LOCAL_COROUTINE"} · uptime {formatUptime(system.data?.uptimeSeconds)}</small></div></div></div>
