...
function DesignTab({ snapshot, message, setMessage, submit, pending, decide }: { snapshot?: Snapshot; message: string; setMessage: (value: string) => void; submit: (event: FormEvent) => void; pending: boolean; decide: (approve: boolean) => void }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(340px,.75fr)]">
    ...
    <section className="space-y-4">
      ...
      {snapshot?.proposal?.agentDesign && <Card title="Agent Package" icon={Bot}><p className="text-sm font-medium">에이전트 생성이 완료되었습니다.</p><p className="mt-2 text-xs leading-5 text-mute">샘플 데이터로 테스트하거나 실행 패키지를 내려받을 수 있습니다. 실제 Slack·Notion 연결에는 사용자의 인증 설정이 필요합니다.</p><div className="mt-3 flex flex-wrap gap-2 text-[11px]"><span className="rounded-pill bg-green-100 px-3 py-1 text-green-800">구조 검증 {snapshot.proposal.agentDesign.review.passed ? "통과" : "수정 필요"}</span><span className="rounded-pill bg-amber-100 px-3 py-1 text-amber-800">{snapshot.proposal.agentDesign.executionReadiness === "EXECUTION_NOT_CONFIGURED" ? "설계 완료 · 실제 실행 미설정" : snapshot.proposal.agentDesign.executionReadiness}</span></div>{snapshot.proposal.agentDesign.review.issues.map((issue) => <p key={issue.code} className="mt-2 text-xs text-mute">· {issue.message}</p>)}{snapshot.currentVersionId && <a href={`/api/builder/workflows/${snapshot.workflowId}/package`} className="mt-4 inline-flex items-center gap-2 rounded-pill bg-ink px-4 py-2 text-xs text-white"><FileText className="h-3.5 w-3.5" />Agent Package 다운로드</a>}</Card>}
