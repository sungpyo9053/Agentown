...
export default function AutomationBuilderPage() {
  ...
  return <AppShell kicker="AGENTOWN · AGENT COMPILER" title="자연어로 AI 에이전트 만들기">
    ...
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border border-hairline bg-white px-5 py-4">
      <div><p className="text-xs font-semibold tracking-[.14em] text-coral">NATURAL LANGUAGE → VALIDATED AGENT PACKAGE</p><p className="mt-1 text-sm text-mute">하고 싶은 일을 설명하면 필요한 에이전트와 도구·워크플로를 설계하고 직접 시험할 수 있는 패키지로 만듭니다.</p></div>
    ...
    <nav aria-label="Builder 화면" className="mb-5 grid grid-cols-4 border border-hairline bg-white p-1">
      ...
      <TabButton active={tab === "production"} onClick={() => setTab("production")} icon={Rocket} label="외부 Runtime" disabled={snapshot?.status !== "ACTIVE"} />
    </nav>
