...
type Snapshot = {
  ...
  packageStatus: "PACKAGE_VALIDATED" | "EXECUTION_NOT_CONFIGURED";
  interactiveStatus: "INTERACTIVE_READY" | "EXECUTION_NOT_CONFIGURED";
  automationStatus: "AUTOMATION_READY" | "EXECUTION_NOT_CONFIGURED";
...
function ReadinessBox({ label, status }: { label: string; status: string }) { const ready = status !== "EXECUTION_NOT_CONFIGURED"; return <div className="rounded-md border border-hairline bg-white p-3"><p className="text-[10px] text-mute">{label}</p><p className={`mt-1 break-all text-[10px] font-semibold ${ready ? "text-emerald-700" : "text-amber-800"}`}>{status}</p></div>; }
