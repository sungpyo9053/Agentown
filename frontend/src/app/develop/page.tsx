...
type Snapshot = {
  ...
  externalInputSchema?: RunInputField[];
...
export default function AgentDevelopmentPage() {
  ...
  if (snapshotQuery.error instanceof ApiError && snapshotQuery.error.status === 401 && sessionId) {
    ...
    return <AppShell kicker="DEVELOP" title="내 에이전트 열기"><div className="mx-auto max-w-lg rounded-lg border border-hairline bg-white p-6"><h1 className="text-lg font-semibold">에이전트를 만든 계정으로 로그인하세요</h1><p className="mt-3 text-sm text-mute">로그인 후 이 에이전트와 실행 방법을 확인할 수 있습니다. 패키지에 따라 내 PC에 도구 설치와 AI 서비스 로그인이 필요합니다.</p><a href={`/login?next=${encodeURIComponent(next)}`} className="mt-5 inline-block rounded-md bg-ink px-4 py-3 text-sm text-white">로그인하고 에이전트 열기</a></div></AppShell>;
  }
