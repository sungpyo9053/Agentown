...
export function AuthForm({mode}:{mode:"login"|"signup"}){
 ...
 return <form onSubmit={submit} className="w-full rounded-2xl border border-zinc-200 bg-white p-8"><p className="text-xs font-semibold uppercase tracking-wide text-coral">Agentown</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">{mode==="login"?"지금 만나요":"새 AI 회사 만들기"}</h1><p className="mt-2 text-sm text-zinc-500">{mode==="login"?"아이디로 내 AI 팀에 들어가세요.":"휴대폰 인증 후 나만의 AI 회사를 시작하세요."}</p><div className="mt-7 space-y-4">
  ...
  <label className="block text-sm font-medium text-zinc-700">아이디<span className="mt-2 flex gap-2"><input className={`${input} mt-0`} autoComplete="username" placeholder="my_village" {...register("handle")}/>{mode==="signup"&&<button type="button" onClick={checkId} className="shrink-0 rounded-lg border border-zinc-200 px-4 text-xs font-semibold text-zinc-700 hover:border-coral hover:text-coral">중복 확인</button>}</span><small className={idCheck?.available?"text-leaf":"text-red-600"}>{errors.handle?.message||idCheck&&(idCheck.available?"사용 가능한 아이디입니다.":"이미 사용 중인 아이디입니다.")}</small></label>
  ...
  {mode==="signup"&&<><label className="block text-sm font-medium text-zinc-700">휴대폰 번호<span className="mt-2 flex gap-2"><input className={`${input} mt-0`} inputMode="tel" placeholder="010-1234-5678" {...register("phone")}/><button type="button" onClick={sendCode} className="shrink-0 rounded-lg border border-zinc-200 px-4 text-xs font-semibold text-zinc-700 hover:border-coral hover:text-coral">인증번호 발송</button></span></label>{verificationId&&<label className="block text-sm font-medium text-zinc-700">인증번호<span className="mt-2 flex gap-2"><input className={`${input} mt-0`} inputMode="numeric" maxLength={6} {...register("verificationCode")}/><button type="button" onClick={verifyCode} className="shrink-0 rounded-lg border border-zinc-200 px-4 text-xs font-semibold text-zinc-700 hover:border-coral hover:text-coral">인증 확인</button></span><small className={phoneVerified?"text-leaf":"text-zinc-500"}>{phoneVerified?"휴대폰 인증이 완료되었습니다.":developmentCode&&`로컬 인증번호: ${developmentCode}`}</small></label>}</>}
 </div>{serverError&&<p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{serverError}</p>}<button disabled={isSubmitting} className="mt-6 w-full rounded-lg bg-coral py-3 font-semibold text-white transition hover:bg-coral/90 disabled:opacity-50">{isSubmitting?"잠시만요…":mode==="login"?"로그인":"인증하고 가입하기"}</button><div className="mt-5 flex justify-center gap-4 text-sm font-medium text-zinc-500">{mode==="login"?<><Link href="/forgot-password" className="hover:text-ink">비밀번호 찾기</Link><Link href="/signup" className="hover:text-ink">회원가입</Link></>:<Link href="/login" className="hover:text-ink">이미 계정이 있어요</Link>}</div></form>;
}
