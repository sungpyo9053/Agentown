...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  return <form onSubmit={submit} className="w-full rounded-2xl border border-zinc-200 bg-white p-8">
    ...
    <div className="mt-7 space-y-4">
      ...
      {mode === "signup" && <>{emailCheck?.available && <button type="button" onClick={sendCode} className="w-full rounded-lg border border-coral py-3 text-sm font-semibold text-coral">이메일 인증번호 발송</button>}{verificationId && <label className="block text-sm font-medium text-zinc-700">인증번호<span className="mt-2 flex gap-2"><input className={`${input} mt-0`} inputMode="numeric" maxLength={6} {...register("verificationCode")} /><button type="button" onClick={verifyCode} className="shrink-0 rounded-lg border border-zinc-200 px-4 text-xs font-semibold text-zinc-700 hover:border-coral hover:text-coral">인증 확인</button></span><small className={emailVerified ? "text-leaf" : "text-zinc-500"}>{emailVerified ? "이메일 인증이 완료되었습니다." : developmentCode && `개발 인증번호: ${developmentCode}`}</small></label>}</>}
