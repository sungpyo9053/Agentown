...
export function AuthForm({mode}:{mode:"login"|"signup"}){
 ...
 async function checkId(){setServerError("");const value=getValues("handle")?.trim()??"";if(!/^[a-z0-9_]{3,30}$/.test(value)){setServerError("아이디 형식을 먼저 확인해 주세요.");return}const result=await api<{handleAvailable?:boolean}>(`/auth/availability?handle=${encodeURIComponent(value)}`);setIdCheck({value,available:result.handleAvailable===true})}
 ...
 return <form onSubmit={submit} className="w-full rounded-[2rem] bg-white p-8 shadow-card"><p className="text-sm font-bold text-coral">AGENT VILLAGE</p><h1 className="mt-2 text-3xl font-black">{mode==="login"?"다시 만나요":"새 AI 회사 만들기"}</h1><p className="mt-2 text-sm text-stone-500">{mode==="login"?"아이디로 내 AI 팀에 들어가세요.":"휴대폰 인증 후 나만의 AI 회사를 시작하세요."}</p><div className="mt-7 space-y-4">
