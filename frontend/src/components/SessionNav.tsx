...
import {clearClarificationDrafts} from "@/lib/clarificationDraft";
...
export function SessionNav({compact=false,showAdminLink=false,hideGreeting=false}:{compact?:boolean;showAdminLink?:boolean;hideGreeting?:boolean}){
 ...
 const logout=useMutation({mutationFn:()=>api<void>("/auth/logout",{method:"POST"}),onSuccess:()=>{try{clearClarificationDrafts(window.sessionStorage)}catch{/* storage may be unavailable */}qc.clear();router.push("/");router.refresh()}});
