...
def test_view_uses_compiled_per_node_output_contract_after_field_renaming(office, tmp_path):
    (tmp_path / 'runtime-definition.json').write_text(json.dumps({'agents': [
        {'name': 'worker-a__review1', 'outputSchema': [{'name': 'delivered'}]},
        {'name': 'worker-a__review2', 'outputSchema': [{'name': 'otherResult'}]},
    ]}))
    viewer = LocalOffice(tmp_path)
    viewer.start()
    try:
        trace = OfficeTrace(viewer)
        raw = json.dumps({'delivered': '실제 최종 표', 'request': 'private input', 'result': 'stale design key'})
        trace.append({'kind': 'agent_end', 'agent': 'worker-a__review1', 'output': raw})
        assert json.loads(viewer.snapshot()['employees'][0]['output']) == {'delivered': '실제 최종 표'}
        assert trace[0]['output'] == raw
        trace.append({'kind': 'agent_end', 'agent': 'worker-a__review2',
                      'output': json.dumps({'otherResult': '다른 실행 결과', 'delivered': 'not this node'})})
        assert json.loads(viewer.snapshot()['employees'][0]['output']) == {'otherResult': '다른 실행 결과'}
    finally:
        viewer.close()
...
def test_schema_input_edits_nested_records_without_json_and_preserves_drafts():
    node = shutil.which('node')
    if not node:
        pytest.skip('Node required for the shipped schema input test')
    html = (Path(__file__).parents[1] / 'agentown_tframex_adapter/office.html').read_text()
    handler = 'function schemaInput' + html.split('function schemaInput', 1)[1].split('function paintInput', 1)[0]
    script = r'''
const assert=require('node:assert/strict');
class Element{
...
const inputForm={dataset:{}};let inputSubmitted=false;const attachFileInput=()=>{};
''' + handler + r'''
const schema={type:'array',maxItems:2,items:{type:'object',required:['id'],properties:{id:{type:'string'},count:{type:'integer'},active:{type:'boolean'},meta:{type:'object',properties:{note:{type:'string'}}}}}};
const form=schemaInput(schema,true),[host,add]=form.element.children;
assert.equal(form.control.value,'');add.onclick();
const fields=host.children[0].children[1].children[0].children;
fields[0].children[1].children[0].value='A <script>';
fields[1].children[1].children[0].value='0';fields[2].children[1].children[0].value='false';
assert.deepEqual(JSON.parse(form.control.value),[{id:'A <script>',count:0,active:false}]);
add.onclick();assert.equal(add.disabled,true);assert.equal(host.children.length,2);
inputForm.dataset.busy='true';host.children[0].children.at(-1).onclick();assert.equal(host.children.length,2);
inputForm.dataset.busy='false';host.children[1].children.at(-1).onclick();assert.equal(host.children.length,1);
assert.equal(JSON.parse(form.control.value)[0].id,'A <script>');
const before=form.control.value;
for(const invalid of ['{}','[{"id":"x","unknown":1}]','[{"count":"wrong"}]','[{},{},{}]']){assert.throws(()=>{form.control.value=invalid});assert.equal(form.control.value,before)}
form.control.value='[{"id":"B","meta":{"note":"nested"}}]';assert.deepEqual(JSON.parse(form.control.value),[{id:'B',meta:{note:'nested'}}]);
form.control.value='[{"id":"C"}]';assert.deepEqual(JSON.parse(form.control.value),[{id:'C'}]);
form.control.value='';assert.equal(form.control.value,'');assert.equal(host.children.length,0);
const optional=schemaInput({type:'object',properties:{rows:schema}},false);assert.equal(optional.control.value,'');
optional.control.value='{"rows":[]}';assert.deepEqual(JSON.parse(optional.control.value),{rows:[]});
optional.control.value='{}';assert.deepEqual(JSON.parse(optional.control.value),{});
'''
    subprocess.run([node, '-e', script], check=True, capture_output=True, text=True)
