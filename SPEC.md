...
* 하네스와 Agent 수를 늘리는 것을 목표로 삼지 않는다.
* 기존 소유자 Agent와 Guide가 동일 책임을 충족하면 재사용한다.
* 단일 책임 업무는 한 명의 Agent로 구성할 수 있어야 한다.
* 새 Agent는 독립 책임, 별도 도구·모델, 독립 검증 또는 실패 격리 필요성이 있을 때만 추가한다.
* 설계 LLM은 변경안을 제안하지만 자신의 결과를 발행 승인할 수 없다.
* 독립 Harness Validator가 시작·종료·순차 흐름·입력 매핑·소유권·재시도·타임아웃·승인·결과 계약을 검사한다.
* 검증을 통과한 불변 HarnessVersion만 실행할 수 있다.
* Execution 생성 시 버전 스냅샷을 고정하며 실행 중 수정 가능한 Harness나 Agent 정의를 다시 읽지 않는다.
* 검증 기록에는 Validator 버전, 구조 해시, 검사 결과와 검증 시각을 남긴다.
* 실패, 변경 없음, 승격 거부와 롤백도 정상적인 엔진 결과로 취급한다.
* 자동 개선은 후보 제안까지만 수행하며 독립 검증과 사용자 승인 없이 운영 버전을 교체하지 않는다.
