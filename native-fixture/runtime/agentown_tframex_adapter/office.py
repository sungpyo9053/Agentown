...
class LocalOffice:
    def __init__(self, root: Path, *, input_schema=None):
        ...
        self.output_fields = {a["key"]: [f["name"] for f in a.get("outputSchema", [])]
                              for a in bundle["agentDefinitions"]}
        runtime_path = root / "runtime-definition.json"
        runtime_agents = json.loads(runtime_path.read_text(encoding="utf-8")).get("agents", []) if runtime_path.is_file() else []
        # Compilation can rename or project outputs independently for each node.
        # The viewer must use the same contract as execution, not stale role fields.
        self.runtime_output_fields = {a["name"]: [f["name"] for f in a["outputSchema"]]
                                      for a in runtime_agents if a.get("name") in self.nodes and "outputSchema" in a}
        ...
        with self.lock:
            ...
            for employee in self.state["employees"]:
                ...
                if kind == "agent_start":
                    ...
                    fields = self.runtime_output_fields.get(node, self.output_fields.get(key))
                    if not failed and fields is not None:
                        try:
                            value = json.loads(output)
                            if isinstance(value, dict):
                                output = json.dumps({field: value[field] for field in fields
