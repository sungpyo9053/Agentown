...
class AgentCompilerGoldenTest {
    ...
    @Test fun `A B C compile into distinct executable graph shapes`() {
        ...
        val shapes = listOf(a, b, c).map { it.proposal.graphPlan!!.nodes.map { node -> node.nodeType } }
        assertThat(shapes.distinct()).hasSize(3)
        assertThat(shapes[0]).contains("knowledge.search.mock", "ai.generate")
        assertThat(shapes[1]).contains("data.csv.compare").doesNotContain("ai.generate", "ai.classify")
        assertThat(b.agentDefinitions).isEmpty()
        assertThat(shapes[2]).contains("schedule.trigger", "human.approval", "slack.send.mock")
        listOf(a, b, c).flatMap { it.proposal.graphPlan!!.edges }.forEach { assertThat(it.bindings).describedAs(it.id).isNotEmpty }
    }

    private fun compile(instruction: String) = pipeline.generateDesign(PipelineContext(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID()), instruction)
}
