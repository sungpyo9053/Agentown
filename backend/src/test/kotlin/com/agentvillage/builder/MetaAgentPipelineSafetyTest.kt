...
class MetaAgentPipelineSafetyTest {
    ...
    fun `manual trigger marker passes declared inputs to text entry without inventing data`() {
        ...
        val triggered = plan.copy(edges = listOf(plan.edges.single().copy(bindings = listOf(WorkflowFieldBinding("triggered", "triggered")))))
        val normalizedTriggered = pipeline.normalizeEntryBindings(triggered, fields)
        assertThat(normalizedTriggered.edges.single().bindings).containsExactlyElementsOf(fields.map { WorkflowFieldBinding(it.name, it.name) })
        assertThat(pipeline.normalizeEntryBindings(normalizedTriggered, fields)).isEqualTo(normalizedTriggered)
        assertThat(pipeline.normalizeEntryBindings(triggered, fields + FieldDefinition("triggered", "boolean", true, "user data"))).isEqualTo(triggered)
