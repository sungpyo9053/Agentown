...
class AgentCompilerVersionGoldenIntegrationTest : IntegrationTestSupport() {
    ...
    @Test fun `follow up replacement creates new version and preserves topology`() {
        ...
        snapshot = service.sendMessage(owner.id, snapshot.conversationId, "Slack 문의를 Notion FAQ에서 찾아 답변 초안을 만들고 담당자 승인 후 원래 Slack 스레드로 전송한다.", "golden-design-$suffix")
        ...
        snapshot = service.applyPatch(owner.id, snapshot.workflowId, "아까 만든 에이전트에서 Slack 전송을 이메일 전송으로 바꿔줘.", originalVersion, snapshot.validation!!.graphHash, "golden-patch-$suffix")
        ...
        assertThat(files.keys).contains("agent.yaml", "workflow.yaml", "tools/tools.yaml", "version.json")
        assertThat(mapper.readTree(files.getValue("version.json"))["workflowVersionId"].asText()).isEqualTo(snapshot.currentVersionId.toString())
        assertThat(files.getValue("tools/tools.yaml")).contains("connector.email.mock", "requires_user_action: true")
    }
}
