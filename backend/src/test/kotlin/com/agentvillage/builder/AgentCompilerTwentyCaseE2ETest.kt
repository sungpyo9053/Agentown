...
class AgentCompilerTwentyCaseE2ETest {
    ...
    @TestFactory
    fun `ten similar and ten distinct natural language requests keep truthful package contracts`() = cases().map { case ->
        DynamicTest.dynamicTest("${case.group}-${case.id}") {
            ...
            assertThat(files.getValue("runtime/pyproject.toml")).contains("23d7a45dd9e2e52f54f44ff8f63c6dff28ef8603")
            assertThat(files.getValue("runners/python/runner.py"))
                .contains("AgentownTFrameXAdapter")
                .doesNotContain("Fixed Agentown mock runner", "Mock research")
            assertThat(files.getValue("runtime-status.json")).containsAnyOf(
                "\"configured\" : true",
    ...
    private fun cases() = listOf(
        Case("S01", "SIMILAR", "FAQ를 찾아 고객 질문에 답하고 근거가 없으면 담당자 검토로 보내는 에이전트", setOf("ai.generate")),
        Case("S02", "SIMILAR", "고객 문의와 관련된 도움말을 검색해 답변 초안을 만들되 자료가 없으면 사람 확인이 필요하다고 표시해줘", setOf("ai.generate")),
        Case("S03", "SIMILAR", "FAQ 근거가 있을 때만 한국어 답변을 작성하고 무근거 질문은 담당자에게 넘겨줘", setOf("ai.generate")),
        Case("S04", "SIMILAR", "지원 문서 검색 결과로 고객 응답을 만들고 관련 문서가 없으면 답하지 않는 에이전트", setOf("ai.generate")),
        Case("S05", "SIMILAR", "고객 질문을 FAQ에서 조회하고 찾은 근거와 답변 또는 상담원 확인 필요 상태를 반환해줘", setOf("ai.generate")),
        Case("S06", "SIMILAR", "두 CSV를 ID 열로 비교해 추가 삭제 수정 행을 표로 보여줘", setOf("ai.generate")),
        Case("S07", "SIMILAR", "엑셀에서 내보낸 CSV 이전본과 최신본의 달라진 레코드를 키 기준으로 찾아줘", setOf("ai.generate")),
        Case("S08", "SIMILAR", "CSV A와 CSV B 사이에 생긴 행 추가 제거 값 변경을 정확하게 비교해줘", setOf("ai.generate")),
        Case("S09", "SIMILAR", "두 데이터 CSV 파일을 기본키 기준으로 diff해서 변경 목록을 만들어줘", setOf("ai.generate")),
        Case("S10", "SIMILAR", "고객 목록 CSV 두 버전을 비교하고 ADDED REMOVED MODIFIED 결과를 출력해줘", setOf("ai.generate")),
        Case("D01", "DISTINCT", "매일 오전 8시에 AI 뉴스 세 개를 요약하고 승인 후 Slack으로 보내줘", setOf("ai.generate", "human.approval")),
        Case("D02", "DISTINCT", "경쟁사 세 곳의 제품 발표와 가격을 병렬 조사해 비교 보고서로 합쳐줘", setOf("ai.generate"), setOf("parallel.map.mock")),
        Case("D03", "DISTINCT", "GitHub 이슈를 버그 기능 요청 질문으로 분류하고 버그 재현 절차를 써줘", setOf("ai.classify")),
        Case("D04", "DISTINCT", "항공권이 20만 원 이하면 결제 전 승인을 요청하는 모니터링 에이전트", setOf("ai.generate", "human.approval")),
        ...
        Case("D09", "DISTINCT", "PeopleMagic에서 휴가 잔여일을 조회해 알려줘", setOf("ai.generate"), setOf("knowledge.search.mock")),
