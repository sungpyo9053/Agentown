...
class AgentCompilerRealGoldenTest {
    ...
    @Test fun `real Codex produces distinct validated packages`() {
        ...
        val translator = WorkflowGraphTranslator(WorkflowNodeCatalog())
        val renderer = HarnessPackageRenderer(mapper)
        val packageRoot = Path.of("build/reports/agent-compiler-real-packages")
        val inputs = linkedMapOf(
            ...
            "H" to "FAQ를 찾아 고객 질문에 답하고 근거가 없으면 담당자 검토로 보내는 에이전트",
        )
        val results = inputs.mapValues { (id, text) ->
            ...
            val graph = translator.translate(UUID.randomUUID(), bundle.proposal)
            ...
            val packageDirectory = packageRoot.resolve(id)
            files.forEach { (path, content) ->
                packageDirectory.resolve(path).also { target -> Files.createDirectories(target.parent); Files.writeString(target, content) }
            }
            if (id == "H") {
                val process = ProcessBuilder("python3", packageDirectory.resolve("runners/python/runner.py").toString()).redirectErrorStream(true).start()
                val output = process.inputStream.bufferedReader().readText()
                assertThat(process.waitFor()).withFailMessage(output).isZero()
                assertThat(output).contains("\"status\": \"SUCCEEDED\"", "\"needsAssigneeReview\": true").doesNotContain("\"draftResponse\"")
            }
        ...
        assertThat(graphs[7]).contains("knowledge.search.mock", "condition.branch", "ai.generate").doesNotContain("human.approval")
