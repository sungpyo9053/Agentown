...
class BuilderService(
    ...
    fun decideDesign(ownerId: UUID, workflowId: UUID, approve: Boolean, idempotencyKey: String): BuilderSnapshot {
        ...
        val nextStep = if (LocalArtifactContract.configured(approvedDesign.proposal) || approvedDesign.proposal.graphPlan?.nodes?.any { it.nodeType == NodeType.LOCAL_WEB_RESEARCH.wireName } == true)
...
internal fun agentDevelopmentPrompt(instruction: String) = """
    ...
    아래 품질 기준을 각 역할의 목적에 맞게 행동·검수 규칙에 반영하세요. 이를 이유로 별도 에이전트나 도구를 추가하지 마세요.
    사용자가 새 기획·설계·후보안을 원하면 실제 안을 만드는 책임을 필요한 역할에 배정하세요. 분석과 검수만 하고 후보안 없이 종료하는 설계는 요청을 충족하지 못합니다.
    원문에 없는 사실을 단정하지 않는 규칙과, 근거·제약을 바탕으로 미승인 제안을 만드는 규칙을 구분하세요. 기획 역할에 '입력에 명시된 내용만 다룬다'는 식으로 제안 자체를 막는 규칙을 넣지 마세요.
    ${AgentOutputQualityPolicy.instructions}
