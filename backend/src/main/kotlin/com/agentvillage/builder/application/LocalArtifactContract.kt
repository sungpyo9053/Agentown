...
object LocalArtifactContract {
    ...
    fun producerInstruction(format: String): String {
        ...
        return "파일 제작 계약: artifactJson 필드에 다음 구조의 유효한 JSON 문자열을 반환한다. $structure " +
            ...
            "파일의 제목·표 머리글·열 제목은 사용자가 요청한 명칭과 언어를 그대로 따른다. 내부 JSON 필드명이나 Agent 키를 사용자용 표시 이름으로 복사하지 않는다. " +
            "XLSX columns에는 사용자용 열 제목 문자열을 쓴다. 내부 키가 필요하면 {\"key\":\"내부 식별자\",\"label\":\"사용자에게 보일 열 제목\"}를 쓰며 label은 필수다. rows는 columns와 같은 순서의 값 배열을 유지한다. " +
