...
object BuilderMvpSupportPolicy {
    ...
    internal fun unsupportedCapabilities(text: String): List<String> {
        ...
        return buildList {
            ...
            val deploymentNegated = listOf(
                "실제 배포는 하지", "실제 배포를 하지", "배포하지 않", "배포하지 마", "배포 없이", "검토 패키지",
            ).any(normalized::contains)
            if (!deploymentNegated) capability(normalized, "개발 도구 쓰기·배포", "jira", "github에 쓰", "깃허브에 쓰", "커밋해", "커밋까지", "자동 배포", "운영 배포", "운영 서버까지")
