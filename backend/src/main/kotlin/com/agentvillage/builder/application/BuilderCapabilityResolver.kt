...
class BuilderCapabilityResolver {
    ...
    private fun binding(node: WorkflowNodePlan): ResourceBinding {
        ...
        return when (NodeType.fromWire(node.nodeType)) {
            ...
            NodeType.NOTION_CREATE_PAGE -> ResourceBinding(
                capability, ResourceKind.CONNECTOR, "connector.notion.oauth", "Notion OAuth",
                ResourceAvailability.INSTALLED, "SERVER_CATALOG", "테넌트 OAuth 연결과 사용자 승인 후 Notion 페이지 생성",
